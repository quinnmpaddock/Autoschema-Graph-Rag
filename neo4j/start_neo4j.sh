...
NEO4J_BASE_DIR="$(pwd)"
export NEO4J_HOME="${NEO4J_BASE_DIR}/app-neo4j-community-${NEO4J_VERSION}"
...
${NEO4J_HOME}/bin/cypher-shell -u neo4j -p admin2025 'CREATE CONSTRAINT n10s_unique_uri FOR (r:Resource) REQUIRE r.uri IS UNIQUE;' -a bolt://localhost:7687
${NEO4J_HOME}/bin/cypher-shell -u neo4j -p admin2025 'call n10s.graphconfig.init({ handleMultival: "OVERWRITE", handleVocabUris: "SHORTEN", keepLangTag: false, handleRDFTypes: "NODES" })' -a bolt://localhost:7687
